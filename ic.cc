...
void IC_Entry::PrintIC(std::ostream & ofs)
{
  ...
  if (label != "") { out_line << label << ": "; 
     blockID = IC_Entry::getBlockID();
    out_line <<std::endl<<"#blockA "<<blockID<< std::endl; }
  else { out_line << "  "; }
  ...
  if (inst != "") {
    ...
    //std::cout<<inst.substr(0,7)<<std::endl;
    if(isJump()){
      blockID = IC_Entry::getBlockID();
      out_line <<std::endl<<"#blockB "<<blockID<< std::endl;
    }
  }
...
void IC_Array::makeBlockVector()
{
  ...
  for (int i = 0; i < (int) ic_array.size(); i++) {
    if(firstBlock == 1){
      blockTrack++;
      for (int y = 0; y < (int) ic_array[i].args.size(); y++) {
        vars.push_back(ic_array[i].args[y].str_value);
        std::cout<<ic_array[i].args[y].str_value<<std::endl;
      }
      firstBlock = 0;
    }else{
      if(ic_array[i].blockID != 0){
        blockTrack ++;
        if(blockTrack != 2){
          for (int y = 0; y < (int) ic_array[i].args.size(); y++) {
            vars.push_back(ic_array[i].args[y].str_value);
...
void IC_Array::tagLocal()
  {
    // int blockTrack = 0;
    for(int i = 0; i< (int) ic_array.size(); i++) {
      for (int y = 0; y < (int) ic_array[i].args.size(); y++) {
        for (int i = 0; i < (int) ic_vars.size(); i++) {
...
//   ofs <<"#block "<<IC_Entry::getBlockID()<< std::endl;
//   for (int i = 0; i < (int) ic_array.size(); i++) {
//     ic_array[i].PrintIC(ofs);
//     // ofs<<ic_array[i].blockID<<std::endl;
//   }
// }
...
void IC_Array::PrintIC(std::ostream & ofs)
{
  ofs << "# Ouput from Dr. Charles Ofria's reference code." << std::endl;
  ofs <<"#block "<<IC_Entry::getBlockID()<< std::endl;
  firstBlock = IC_Array::getFirstBlock();
  for (int i = 0; i < (int) ic_array.size(); i++) {
    ic_array[i].PrintIC(ofs);
    // ofs<<ic_array[i].blockID<<std::endl;
  }
